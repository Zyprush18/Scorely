...
type testRepoMajor struct {
	StupCtx func() (context.Context, context.CancelFunc)
	Name string
	Id   int
...
func TestGetAll(t *testing.T)  {
	...
	data := []testRepoMajor{
		...
		},
		{
			...
			Name: "Failed Get All Data Context Timeout",
			DataRows: sqlmock.NewRows([]string{
				"id_major",
				"major",
				"major_abbreviation",
			...
			FindDataAll: func(src string, perpage, offset int, data *sqlmock.Rows) *sqlmock.ExpectedQuery {
				if offset != 0 {
					return mock.ExpectQuery(regexp.QuoteMeta("SELECT * FROM `majors` WHERE major LIKE ? ORDER BY created_at ASC LIMIT ? OFFSET ?")).WithArgs("%"+src+"%", perpage,offset).WillReturnError(context.DeadlineExceeded)
				}
				return mock.ExpectQuery(regexp.QuoteMeta("SELECT * FROM `majors` WHERE major LIKE ? ORDER BY created_at ASC LIMIT ?")).WithArgs("%"+src+"%", perpage).WillReturnError(context.DeadlineExceeded)
			},
			Page: 1,
			Perpage: 10,
			Search: "",
	...
	for _, v := range data {
		t.Run(v.Name, func(t *testing.T) {
			...
			ctx, cancel := v.StupCtx()
			defer cancel()

			resp, count, err := repomajor.GetAllData(ctx,v.Search,v.Sort,v.Page,v.Perpage)
			if v.IsErr {
				assert.Error(t, err)
				assert.Nil(t, resp)
			} else {
